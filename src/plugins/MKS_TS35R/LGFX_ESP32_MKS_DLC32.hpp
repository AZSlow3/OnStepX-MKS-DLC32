...
//*/
    ...
    { // 表示パネル制御の設定を行います。
      ...
      cfg.pin_cs           =    25;  // CSが接続されているピン番号   (-1 = disable)
      cfg.pin_rst          =    27;  // RSTが接続されているピン番号  (-1 = disable)
...
//*
    { // バックライト制御の設定を行います。（必要なければ削除）
      ...
      cfg.invert = true;            // AZ: TRUE for MKS-DLC32  // バックライトの輝度を反転させる場合 true
...
//*
    { // タッチスクリーン制御の設定を行います。（必要なければ削除）
      ...
      // AZ: from calibration / observing raw numbers
      cfg.x_min      = 230;    // 252 タッチスクリーンから得られる最小のX値(生の値)
      cfg.x_max      = 3820; //3684  // タッチスクリーンから得られる最大のX値(生の値)
      cfg.y_min      = 170;    // 171 タッチスクリーンから得られる最小のY値(生の値)
      cfg.y_max      = 3900; //3726  // タッチスクリーンから得られる最大のY値(生の値)
